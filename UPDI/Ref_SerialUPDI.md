...
* The FT232 requires you to change the timeout setting to get acceptable performance.
  ...
  * It is probably not coincidental that most FT232 adapters from China are also of noticeably poor build quality. You can see the chip not centered, the passives slightly crooked, the solder looks duller than it should, I've broken off a capacitor during normal handlingm and none of the solder joints inspirea confidence. They typically work fine, and some of them even have a 6-pin ISP header on them. (huh??? Well it's so you can progeram classic AVRs in bitbang mode....). But they really don't inspire confidence.
  * The FT232RL's like the other 3.3v serial adapters (the ones with 3.3v logic level outputs, evem if the Vcc pin is suppolying 5 to the target) have trouble above 230400 baud, and sometimes even there - **HOWEVER** there is a solution to a FT232RL which doesn't program except at lower speeds. That solution to *only use the schottky diode method* and *in addition to that, connect a 10k resistor between the RX line and the 5v Vcc pin* FT232RL is 5v tolerant. So this will not harm it. Most of those chart topping numbers in this document for the FT232RL were generated in that way. 
...
Suopose someone has 2 hex files. Assume they both fill the flash. This individual gives you one file. The file is either one of the two hex files, or it the result of combining the files using some bitwise operatoer (except for the interrupt vectors at the start). The original programs are both very useful to you, and you've wired up hardware to his specifications. He swears that the hex file he gave you is one of the real program and works just great, and if it doesn't work for you, you must not have wired it right. What operation would he need to use to make the fake most believable?
